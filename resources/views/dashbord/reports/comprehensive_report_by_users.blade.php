...
@section('toolbar')
    <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
        @php
            $title = trans('reports.comprehensive_report_by_users') ?? 'تقرير شامل بالمستخدمين';
            $breadcrumbs = [
    ...
    <div id="kt_app_content_container" class="app-container container-xxxl">
        ...
        <div class="card shadow-sm" style="border-top: 3px solid #28a745;">
            <div class="card-header">
                <h3 class="card-title">تقرير شامل بالمستخدمين</h3>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped table-bordered" id="users_report_table">
                        ...
                        <tbody id="report_tbody">
                            <tr>
                                <td colspan="4" class="text-center">اضغط على زر البحث لعرض التقرير</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#reset_btn').on('click', function() {
                $('#from_date').val('');
                $('#to_date').val('');
                $('#report_tbody').html('<tr><td colspan="4" class="text-center">اضغط على زر البحث لعرض التقرير</td></tr>');
                $('#totals_section').hide();
            });
            ...
            function loadComprehensiveReport() {
                ...
                $.ajax({
                    url: "{{ route('admin.reports.comprehensive_by_users_data') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        from_date: fromDate,
            ...
            function displayReport(data) {
                ...
                if (data.users && data.users.length > 0) {
                    ...
                    tbody.append('<tr><td colspan="4" class="text-center">لا توجد بيانات للفترة المحددة</td></tr>');
                }
