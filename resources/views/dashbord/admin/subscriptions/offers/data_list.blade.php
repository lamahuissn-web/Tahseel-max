...
@section('toolbar')
    <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
        <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
            <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">{{trans('sub.offers')}}</h1>
            <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                ...
                <li class="breadcrumb-item"><span class="bullet bg-gray-400 w-5px h-2px"></span></li>
                <li class="breadcrumb-item text-muted">{{trans('sub.offers')}}</li>
                <li class="breadcrumb-item"><span class="bullet bg-gray-400 w-5px h-2px"></span></li>
                <li class="breadcrumb-item text-muted">{{trans('sub.offers_table')}}</li>
            </ul>
        </div>

        <div class="d-flex align-items-center gap-2 gap-lg-3">
            <div class="d-flex">
                <a href="{{route('admin.subscriptions.offers.create')}}"
        ...
        <div id="kt_app_content_container" class="t_container" >
            <div class="card shadow-sm ">
                <div class="card-header">
                    <h3 class="card-title"></i> {{trans('sub.offers_table')}}</h3>
                    @if ($errors->any())
                        ...
                        <div class="table-responsive">
                            <table id="table" class="table table-bordered">
                                ...
                                <tr class="fw-bold fs-6 text-gray-800">
                                    ...
                                    <th>{{trans('sub.offer_type')}}</th>
                                    <th>{{trans('sub.customize_to')}}</th>
                                    <th>{{trans('sub.price')}}</th>
                                    <th>{{trans('sub.duration')}}</th>
                                    <th>{{trans('sub.status')}}</th>
        ...
        var KTDatatablesServerSide = function () {
            ...
            var initDatatable = function () {
                dt = $('#table').DataTable({
                    ...
                    ajax: "{{route('admin.subscriptions.get_ajax_offers')}}",
                    columns: [
                        { data: 'id', className: 'text-center' },
                        { data: 'title', className: 'text-center' },
                        { data: 'offer_type', className: 'text-center' },
                        { data: 'customize_to', className: 'text-center' },
